...
function activateScriptElement(element2) {
...
function getAttribute(attributeName, ...elements) {
  for (const value of elements.map((element2) => element2?.getAttribute(attributeName))) {
    if (typeof value == "string") return value;
  }
  return null;
}
function hasAttribute(attributeName, ...elements) {
  return elements.some((element2) => element2 && element2.hasAttribute(attributeName));
}
...
function waitForLoad(element2, timeoutInMilliseconds = 2e3) {
  return new Promise((resolve) => {
    const onComplete = () => {
      element2.removeEventListener("error", onComplete);
      element2.removeEventListener("load", onComplete);
      resolve();
    };
    element2.addEventListener("load", onComplete, { once: true });
    element2.addEventListener("error", onComplete, { once: true });
...
function getMetaContent(name) {
  const element2 = getMetaElement(name);
  return element2 && element2.content;
}
function setMetaContent(name, content) {
  let element2 = getMetaElement(name);
  if (!element2) {
    element2 = document.createElement("meta");
...
function findClosestRecursively(element2, selector) {
  if (element2 instanceof Element) {
    return element2.closest(selector) || findClosestRecursively(element2.assignedSlot || element2.getRootNode()?.host, selector);
  }
}
function elementIsFocusable(element2) {
  const inertDisabledOrHidden = "[inert], :disabled, [hidden], details:not([open]), dialog:not([open])";
  return !!element2 && element2.closest(inertDisabledOrHidden) == null && typeof element2.focus == "function";
}
...
function doesNotTargetIFrame(anchor) {
  if (anchor.hasAttribute("target")) {
    for (const element2 of document.getElementsByName(anchor.target)) {
      if (element2 instanceof HTMLIFrameElement) return false;
    }
  }
  return true;
}
...
function importStreamElements(fragment) {
  for (const element2 of fragment.querySelectorAll("turbo-stream")) {
    const streamElement = document.importNode(element2, true);
    for (const inertScriptElement of streamElement.templateElement.content.querySelectorAll("script")) {
      inertScriptElement.replaceWith(activateScriptElement(inertScriptElement));
    }
    element2.replaceWith(streamElement);
  }
  return fragment;
}
...
function submissionDoesNotTargetIFrame(form, submitter) {
  if (submitter?.hasAttribute("formtarget") || form.hasAttribute("target")) {
    ...
    for (const element2 of document.getElementsByName(target)) {
      if (element2 instanceof HTMLIFrameElement) return false;
    }
    return true;
  } else {
...
var FormLinkClickObserver = class {
  constructor(delegate, element2) {
    this.delegate = delegate;
    this.linkInterceptor = new LinkClickObserver(this, element2);
  }
  start() {
...
var FrameRenderer = class extends Renderer {
  ...
  scrollFrameIntoView() {
    if (this.currentElement.autoscroll || this.newElement.autoscroll) {
      ...
      if (element2) {
        element2.scrollIntoView({ block, behavior });
        return true;
      }
    }
    return false;
...
var ProgressBar = class _ProgressBar {
  ...
  createStylesheetElement() {
    ...
    element2.textContent = _ProgressBar.defaultCSS;
    if (this.cspNonce) {
      element2.nonce = this.cspNonce;
    }
    return element2;
  }
...
var HeadSnapshot = class extends Snapshot {
  detailsByOuterHTML = this.children.filter((element2) => !elementIsNoscript(element2)).map((element2) => elementWithoutNonce(element2)).reduce((result, element2) => {
    ...
    return {
      ...
      [outerHTML]: {
        ...details,
        elements: [...details.elements, element2]
      }
    };
  }, {});
...
var CacheObserver = class {
  ...
  removeTemporaryElements = (_event) => {
    for (const element2 of this.temporaryElements) {
      element2.remove();
    }
  };
...
var FrameRedirector = class {
  constructor(session2, element2) {
    this.session = session2;
    this.element = element2;
    this.linkInterceptor = new LinkInterceptor(this, element2);
    this.formSubmitObserver = new FormSubmitObserver(this, element2);
  }
  start() {
    this.linkInterceptor.start();
  ...
  shouldInterceptLinkClick(element2, _location, _event) {
    return this.#shouldRedirect(element2);
  }
  linkClickIntercepted(element2, url, event) {
    const frame = this.#findFrameElement(element2);
    if (frame) {
      frame.delegate.linkClickIntercepted(element2, url, event);
    }
  }
  // Form submit observer delegate
  willSubmitForm(element2, submitter) {
    return element2.closest("turbo-frame") == null && this.#shouldSubmit(element2, submitter) && this.#shouldRedirect(element2, submitter);
  }
  formSubmitted(element2, submitter) {
    const frame = this.#findFrameElement(element2, submitter);
    if (frame) {
      frame.delegate.formSubmitted(element2, submitter);
    }
  }
  ...
  #shouldRedirect(element2, submitter) {
    const isNavigatable = element2 instanceof HTMLFormElement ? this.session.submissionIsNavigatable(element2, submitter) : this.session.elementIsNavigatable(element2);
    if (isNavigatable) {
      const frame = this.#findFrameElement(element2, submitter);
  ...
  #findFrameElement(element2, submitter) {
    const id = submitter?.getAttribute("data-turbo-frame") || element2.getAttribute("data-turbo-frame");
    if (id && id != "_top") {
...
var ErrorRenderer = class extends Renderer {
  ...
  activateScriptElements() {
    for (const replaceableElement of this.scriptElements) {
      ...
      if (parentNode) {
        const element2 = activateScriptElement(replaceableElement);
...
var PageRenderer = class extends Renderer {
  ...
  async copyNewHeadStylesheetElements() {
    ...
    for (const element2 of this.newHeadStylesheetElements) {
      loadingElements.push(waitForLoad(element2));
      document.head.appendChild(element2);
    }
  ...
  copyNewHeadScriptElements() {
    for (const element2 of this.newHeadScriptElements) {
      document.head.appendChild(activateScriptElement(element2));
    }
  }
  removeUnusedDynamicStylesheetElements() {
    for (const element2 of this.unusedDynamicStylesheetElements) {
  ...
  async mergeProvisionalElements() {
    ...
    for (const element2 of this.currentHeadProvisionalElements) {
      if (!this.isCurrentElementInElementList(element2, newHeadElements)) {
        document.head.removeChild(element2);
      }
    }
    for (const element2 of newHeadElements) {
      document.head.appendChild(element2);
    }
  }
  isCurrentElementInElementList(element2, elementList) {
    for (const [index, newElement] of elementList.entries()) {
      if (element2.tagName == "TITLE") {
        if (newElement.tagName != "TITLE") {
          continue;
        }
        if (element2.innerHTML == newElement.innerHTML) {
          elementList.splice(index, 1);
          return true;
        }
      }
      if (newElement.isEqualNode(element2)) {
        elementList.splice(index, 1);
        return true;
      }
    }
  ...
  get unusedDynamicStylesheetElements() {
    return this.oldHeadStylesheetElements.filter((element2) => {
      return element2.getAttribute("data-turbo-track") === "dynamic";
    });
  }
...
var MorphRenderer = class extends PageRenderer {
  ...
  #isFrameReloadedWithMorph(element2) {
    return element2.src && element2.refresh === "morph";
  }
  #remoteFrames() {
...
var Preloader = class {
  ...
  preloadOnLoadLinksForView(element2) {
    for (const link of element2.querySelectorAll(this.selector)) {
...
var Session = class {
  ...
  // Preloader delegate
  shouldPreloadLink(element2) {
    const isUnsafe = element2.hasAttribute("data-turbo-method");
    const isStream = element2.hasAttribute("data-turbo-stream");
    const frameTarget = element2.getAttribute("data-turbo-frame");
    const frame = frameTarget == "_top" ? null : document.getElementById(frameTarget) || findClosestRecursively(element2, "turbo-frame:not([disabled])");
    if (isUnsafe || isStream || frame instanceof FrameElement) {
      return false;
    } else {
      const location2 = new URL(element2.href);
      return this.elementIsNavigatable(element2) && locationIsVisitable(location2, this.snapshot.rootLocation);
    }
  }
  // History delegate
  ...
  allowsImmediateRender({ element: element2 }, options) {
    const event = this.notifyApplicationBeforeRender(element2, options);
    const {
      defaultPrevented,
      detail: { render }
    } = event;
  ...
  preloadOnLoadLinksForView(element2) {
    this.preloader.preloadOnLoadLinksForView(element2);
  }
  ...
  elementIsNavigatable(element2) {
    const container = findClosestRecursively(element2, "[data-turbo]");
    const withinFrame = findClosestRecursively(element2, "turbo-frame");
...
var FrameController = class {
  ...
  elementAppearedInViewport(element2) {
    this.proposeVisitIfNavigatedWithAction(element2, getVisitAction(element2));
    this.#loadSourceURL();
  }
  ...
  linkClickIntercepted(element2, location2) {
    this.#navigateFrame(element2, location2);
  }
  // Form submit observer delegate
  willSubmitForm(element2, submitter) {
    return element2.closest("turbo-frame") == this.element && this.#shouldInterceptNavigation(element2, submitter);
  }
  formSubmitted(element2, submitter) {
    if (this.formSubmission) {
      this.formSubmission.stop();
  ...
  preloadOnLoadLinksForView(element2) {
    session.preloadOnLoadLinksForView(element2);
  }
  viewInvalidated() {
  }
  ...
  visitCachedSnapshot = ({ element: element2 }) => {
  ...
  #navigateFrame(element2, url, submitter) {
    const frame = this.#findFrameElement(element2, submitter);
    frame.delegate.proposeVisitIfNavigatedWithAction(frame, getVisitAction(submitter, element2, frame));
    this.#withCurrentNavigationElement(element2, () => {
      frame.src = url;
    });
  }
  ...
  async extractForeignFrameElement(container) {
    ...
    try {
      element2 = activateElement(container.querySelector(`turbo-frame#${id}`), this.sourceURL);
      if (element2) {
        return element2;
      }
      element2 = activateElement(container.querySelector(`turbo-frame[src][recurse~=${id}]`), this.sourceURL);
      if (element2) {
  ...
  #shouldInterceptNavigation(element2, submitter) {
    ...
    if (!session.elementIsNavigatable(element2)) {
      return false;
    }
  ...
  #withCurrentNavigationElement(element2, callback) {
    this.currentNavigationElement = element2;
    callback();
...
var StreamElement = class _StreamElement extends HTMLElement {
  ...
  get targetElementsById() {
    const element2 = this.ownerDocument?.getElementById(this.target);
    if (element2 !== null) {
      return [element2];
    } else {
      return [];
...
(() => {
  ...
  if (!element2) return;
  if (element2.hasAttribute("data-turbo-suppress-warning")) return;
...
var defaultActionDescriptorFilters = {
  ...
  self({ event, value, element: element2 }) {
    if (value) {
      return element2 === event.target;
    } else {
      return true;
    }
  }
...
var Action = class {
  constructor(element2, index, descriptor, schema) {
    this.element = element2;
    this.index = index;
...
var Binding = class {
  ...
  applyEventModifiers(event) {
    const { element: element2 } = this.action;
    ...
    for (const [name, value] of Object.entries(this.eventOptions)) {
      if (name in actionDescriptorFilters) {
        ...
        passes = passes && filter({ name, value, event, element: element2, controller });
      } else {
        continue;
      }
    }
  ...
  invokeWithEvent(event) {
    ...
    try {
      ...
      const { identifier, controller, element: element2, index } = this;
      const detail = { identifier, controller, element: element2, index, event };
...
var ElementObserver = class {
  ...
  refresh() {
    if (this.started) {
      ...
      for (const element2 of Array.from(this.elements)) {
        if (!matches.has(element2)) {
          this.removeElement(element2);
        }
      }
      for (const element2 of Array.from(matches)) {
  ...
  processAttributeChange(element2, attributeName) {
...
var SelectorObserver = class {
  constructor(element2, selector, delegate, details) {
  ...
  matchElement(element2) {
    const { selector } = this;
    if (selector) {
      const matches = element2.matches(selector);
      if (this.delegate.selectorMatchElement) {
        return matches && this.delegate.selectorMatchElement(element2, this.details);
      }
      return matches;
    } else {
      return false;
  ...
  elementMatched(element2) {
    const { selector } = this;
    if (selector) {
      this.selectorMatched(element2, selector);
    }
  }
  elementUnmatched(element2) {
    const selectors = this.matchesByElement.getKeysForValue(element2);
    for (const selector of selectors) {
      this.selectorUnmatched(element2, selector);
    }
  }
  elementAttributeChanged(element2, _attributeName) {
    const { selector } = this;
    if (selector) {
      const matches = this.matchElement(element2);
      const matchedBefore = this.matchesByElement.has(selector, element2);
  ...
  selectorMatched(element2, selector) {
    this.delegate.selectorMatched(element2, selector, this.details);
    this.matchesByElement.add(selector, element2);
  }
  selectorUnmatched(element2, selector) {
    this.delegate.selectorUnmatched(element2, selector, this.details);
    this.matchesByElement.delete(selector, element2);
  }
};
...
function parseTokenString(tokenString, element2, attributeName) {
  return tokenString.trim().split(/\s+/).filter((content) => content.length).map((content, index) => ({ element: element2, attributeName, content, index }));
}
...
var OutletObserver = class {
  ...
  selectorMatched(element2, _selector, { outletName }) {
    const outlet = this.getOutlet(element2, outletName);
    if (outlet) {
      this.connectOutlet(outlet, element2, outletName);
    }
  }
  selectorUnmatched(element2, _selector, { outletName }) {
    const outlet = this.getOutletFromMap(element2, outletName);
    if (outlet) {
      this.disconnectOutlet(outlet, element2, outletName);
    }
  }
  selectorMatchElement(element2, { outletName }) {
    const selector = this.selector(outletName);
    const hasOutlet = this.hasOutlet(element2, outletName);
    const hasOutletController = element2.matches(`[${this.schema.controllerAttribute}~=${outletName}]`);
    if (selector) {
      return hasOutlet && hasOutletController && element2.matches(selector);
    } else {
      return false;
    }
  }
  ...
  connectOutlet(outlet, element2, outletName) {
    var _a;
    if (!this.outletElementsByName.has(outletName, element2)) {
      this.outletsByName.add(outletName, outlet);
      this.outletElementsByName.add(outletName, element2);
      (_a = this.selectorObserverMap.get(outletName)) === null || _a === void 0 ? void 0 : _a.pause(() => this.delegate.outletConnected(outlet, element2, outletName));
    }
  }
  disconnectOutlet(outlet, element2, outletName) {
    var _a;
    if (this.outletElementsByName.has(outletName, element2)) {
      this.outletsByName.delete(outletName, outlet);
      this.outletElementsByName.delete(outletName, element2);
      (_a = this.selectorObserverMap.get(outletName)) === null || _a === void 0 ? void 0 : _a.pause(() => this.delegate.outletDisconnected(outlet, element2, outletName));
    }
  }
  disconnectAllOutlets() {
    for (const outletName of this.outletElementsByName.keys) {
      for (const element2 of this.outletElementsByName.getValuesForKey(outletName)) {
  ...
  hasOutlet(element2, outletName) {
    return !!this.getOutlet(element2, outletName) || !!this.getOutletFromMap(element2, outletName);
  }
  getOutlet(element2, outletName) {
    return this.application.getControllerForElementAndIdentifier(element2, outletName);
  }
  getOutletFromMap(element2, outletName) {
    return this.outletsByName.getValuesForKey(outletName).find((outlet) => outlet.element === element2);
  }
  get scope() {
    return this.context.scope;
  }
...
var Context = class {
  ...
  outletConnected(outlet, element2, name) {
    this.invokeControllerMethod(`${namespaceCamelize(name)}OutletConnected`, outlet, element2);
  }
  outletDisconnected(outlet, element2, name) {
    this.invokeControllerMethod(`${namespaceCamelize(name)}OutletDisconnected`, outlet, element2);
  }
...
var OutletSet = class {
  ...
  findElement(selector, outletName) {
    ...
    return elements.filter((element2) => this.matchesElement(element2, selector, outletName))[0];
  }
  findAllElements(selector, outletName) {
    const elements = this.scope.queryElements(selector);
    return elements.filter((element2) => this.matchesElement(element2, selector, outletName));
  }
  matchesElement(element2, selector, outletName) {
    const controllerAttribute = element2.getAttribute(this.scope.schema.controllerAttribute) || "";
    return element2.matches(selector) && controllerAttribute.split(" ").includes(outletName);
  }
};
var Scope = class _Scope {
  constructor(schema, element2, identifier, logger) {
    ...
    this.outlets = new OutletSet(this.documentScope, element2);
  }
...
var ScopeObserver = class {
  ...
  parseValueForToken(token) {
    ...
    return this.parseValueForElementAndIdentifier(element2, identifier);
  }
  parseValueForElementAndIdentifier(element2, identifier) {
...
var Router = class {
  ...
  proposeToConnectScopeForElementAndIdentifier(element2, identifier) {
    const scope = this.scopeObserver.parseValueForElementAndIdentifier(element2, identifier);
    if (scope) {
      this.scopeObserver.elementMatchedValue(scope.element, scope);
    } else {
      console.error(`Couldn't find or create scope for identifier: "${identifier}" and element:`, element2);
    }
  }
  handleError(error2, message, detail) {
...
var Application = class {
  ...
  static start(element2, schema) {
    const application2 = new this(element2, schema);
    application2.start();
    return application2;
  }
  async start() {
...
function getOutletController(controller, element2, identifier) {
  return controller.application.getControllerForElementAndIdentifier(element2, identifier);
}
function getControllerAndEnsureConnectedScope(controller, element2, outletName) {
  let outletController = getOutletController(controller, element2, outletName);
  if (outletController)
    return outletController;
  controller.application.router.proposeToConnectScopeForElementAndIdentifier(element2, outletName);
  outletController = getOutletController(controller, element2, outletName);
  if (outletController)
    return outletController;
}
...
var isVisible = (element2) => {
  if (!isElement2(element2) || element2.getClientRects().length === 0) {
    return false;
  }
...
var Config = class {
  ...
  _mergeConfigObj(config, element2) {
    const jsonConfig = isElement2(element2) ? Manipulator.getDataAttribute(element2, "config") : {};
    return {
      ...this.constructor.Default,
      ...typeof jsonConfig === "object" ? jsonConfig : {},
      ...isElement2(element2) ? Manipulator.getDataAttributes(element2) : {},
...
var Collapse = class _Collapse extends BaseComponent {
  ...
  show() {
    ...
    let activeChildren = [];
    if (this._config.parent) {
      activeChildren = this._getFirstLevelChildren(SELECTOR_ACTIVES).filter((element2) => element2 !== this._element).map((element2) => _Collapse.getOrCreateInstance(element2, {
        toggle: false
      }));
    }
  ...
  _isShown(element2 = this._element) {
    return element2.classList.contains(CLASS_NAME_SHOW$7);
  }
  // Private
  _configAfterMerge(config) {
  ...
  _getFirstLevelChildren(selector) {
    ...
    return SelectorEngine.find(selector, this._config.parent).filter((element2) => !children.includes(element2));
  }
...
EventHandler.on(document, EVENT_CLICK_DATA_API$4, SELECTOR_DATA_TOGGLE$4, function(event) {
  ...
  for (const element2 of SelectorEngine.getMultipleElementsFromSelector(this)) {
    Collapse.getOrCreateInstance(element2, {
      toggle: false
    }).toggle();
  }
});
...
var Dropdown = class _Dropdown extends BaseComponent {
  ...
  _selectMenuItem({
    ...
    const items = SelectorEngine.find(SELECTOR_VISIBLE_ITEMS, this._menu).filter((element2) => isVisible(element2));
    if (!items.length) {
      return;
    }
...
EventHandler.on(window, EVENT_RESIZE, () => {
  for (const element2 of SelectorEngine.find("[aria-modal][class*=show][class*=offcanvas-]")) {
...
var Tab = class _Tab extends BaseComponent {
  ...
  _activate(element2, relatedElem) {
    ...
    const complete = () => {
      if (element2.getAttribute("role") !== "tab") {
        element2.classList.add(CLASS_NAME_SHOW$1);
        return;
      }
      element2.removeAttribute("tabindex");
      element2.setAttribute("aria-selected", true);
      this._toggleDropDown(element2, true);
      EventHandler.trigger(element2, EVENT_SHOWN$1, {
  ...
  _deactivate(element2, relatedElem) {
    ...
    const complete = () => {
      if (element2.getAttribute("role") !== "tab") {
        element2.classList.remove(CLASS_NAME_SHOW$1);
        return;
      }
      element2.setAttribute("aria-selected", false);
      element2.setAttribute("tabindex", "-1");
      this._toggleDropDown(element2, false);
      EventHandler.trigger(element2, EVENT_HIDDEN$1, {
        relatedTarget: relatedElem
      });
    };
    this._queueCallback(complete, element2, element2.classList.contains(CLASS_NAME_FADE$1));
...
EventHandler.on(window, EVENT_LOAD_DATA_API, () => {
  for (const element2 of SelectorEngine.find(SELECTOR_DATA_TOGGLE_ACTIVE)) {
